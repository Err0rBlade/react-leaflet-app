...
class Routing extends MapLayer {
  createLeafletElement() {
    let { map } = this.props;
    // waypoints are default starting and destination co-ords
    ...
    const startIcon = L.icon({
      ...
      iconSize: [40, 40],
      iconAnchor: [20, 40], //anchor is the point of the icon image which rests over the actual location on map
      shadowSize: [50, 40],
      shadowAnchor: [15, 40],
    });
    //destination icon
    ...
    let leafletElement = L.Routing.control({
      waypoints,
      //generates the markers, looks up the waypoints array i, startIcon at i=0, destIcon at last waypoint
      ...
      reverseWaypoints: true, //provides a button to reverse the waypoints array
      showAlternatives: true, //shows alternative routes
      addWaypoints: true, //adds a '+' button to add waypoints
      lineOptions: {
        styles: [
          //mainroute style
      ...
      //geocoding codes the coordinates to the location name
      //reverse geocoding converts location names into coordinates
      geocoder: L.Control.Geocoder.nominatim(),
      routeWhileDragging: true, //route array is being updated while marker is being dragged
